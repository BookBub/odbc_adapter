  ...
  module ConnectionAdapters
    class ODBCAdapter < AbstractAdapter
      ...
      def new_column(name, default, sql_type_metadata, null, native_type = nil)
        if name == "letters"
          puts "calling new column on letters"
          puts "name: #{name}"
          puts "default: #{default}"
          puts "metadata:"
          puts " -  sql type: #{sql_type_metadata.sql_type}"
          puts " -  type: #{sql_type_metadata.type}"
          puts " -  limit: #{sql_type_metadata.limit}"
          puts " -  precision: #{sql_type_metadata.precision}"
          puts " -  scale: #{sql_type_metadata.scale}"
          puts "null: #{null}"
          puts "native type: #{native_type}"
          puts "============"
        end
